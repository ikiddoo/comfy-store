...
const Filters = () => {
  // destruct filters from filter context
  const { filters: { text, category, company, color, price, min_price, max_price, shipping },
            ...
            <div className='colors'>
              {
                colors.map((c, index) => {
                  ...
                  return (
                    <button key={index} name='color' onClick={updateFilters} style={{ background: c }} data-color={c}
                      className={`${color === c ? 'color-btn active' : 'color-btn'}`}>{color === c ? <FaCheck /> : null}</button>
                  )
                })
              }
            </div>
